...
class CDCNeXt(nn.Module):
    r""" ConvCDNext
        The Architecture based on `A ConvNet for the 2020s` and `Searching Central Difference Convolutional Networks for Face Anti-Spoofing`
        Source paper:
        - https://arxiv.org/pdf/2003.04092v1.pdf (CDCN)
    ...
    def __init__(self, in_chans=3, num_classes=1, 
        ...
        self.downsample_layers = nn.ModuleList() # stem and 3 intermediate downsampling conv layers
        stem = nn.Sequential(
            Conv2d_cd(in_chans, dims[0], kernel_size=4, stride=4), # Using CD Convolution instead vanilla convolution
            LayerNorm(dims[0], eps=1e-6, data_format="channels_first")
        )
        self.downsample_layers.append(stem)
        for i in range(3):
